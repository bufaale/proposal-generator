...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("[generate] auth ok", Date.now() - t0, "ms");
    ...
    console.log("[generate] plan:", plan.id, "model:", plan.limits.ai_model, Date.now() - t0, "ms");
    ...
    let templateSections = [
      ...
      },
      {
        type: "timeline",
        title: "Timeline",
        prompt_hint: "Project timeline",
      },
      {
        type: "pricing",
        title: "Investment",
        prompt_hint: "Pricing details",
      },
      {
        type: "terms",
    ...
    if (brief.template_id) {
      ...
      if (template?.sections_schema) {
        templateSections =
          template.sections_schema as typeof templateSections;
      }
    }

    console.log("[generate] calling AI...", Date.now() - t0, "ms");
    ...
    console.log("[generate] AI done", Date.now() - t0, "ms");
    ...
    console.log("[generate] done", Date.now() - t0, "ms");
